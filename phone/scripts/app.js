...
$(document).ready(function() {
    ...
    ctxSip = {
        ...
        formatPhone : function(phone) {
			...
			// 기능코드 출력시 특수문자 포함 /[^0-9]/g ---> /[^0-9*]/g
            num = num.toString().replace(/[^0-9*]/g, '');
            
            // OLSSOO Number looks better start
            //if (num.length === 10) {
            ...
            //}
            return num;
            // OLSSOO Number looks better end
        },
        ...
        logItem : function(item) {
            ...
            i  = '<div class="list-group-item sip-logitem clearfix '+callClass+'" data-uri="'+item.uri+'" data-sessionid="'+item.id+'" title="재발신 하시려면 두번클릭하세요">';
            i += '<div class="clearfix"><div class="pull-left">';
            i += '<i class="fa fa-fw '+callIcon+' fa-fw"></i> <strong>'+ctxSip.formatPhone(item.uri)+'</strong><br><small>'+moment(item.start).format('YYYY/MM/DD hh:mm:ss a')+'</small>';
            i += '</div>';
            ...
            if (callActive) {
                ...
                if (item.status === 'ringing' && item.flow === 'incoming') {
                    i += '<button class="btn btn-xs btn-success btnCall" title="받기"><i class="fa fa-phone"></i></button>';
                } else {
                    i += '<button class="btn btn-xs btn-primary btnHoldResume" title="보류"><i class="fa fa-pause"></i></button>';
                    i += '<button class="btn btn-xs btn-info btnTransfer" title="돌려주기"><i class="fa fa-random"></i></button>';
                    i += '<button class="btn btn-xs btn-warning btnMute" title="음소거"><i class="fa fa-fw fa-microphone"></i></button>';
                }
                i += '<button class="btn btn-xs btn-danger btnHangUp" title="끊기"><i class="fa fa-stop"></i></button>';
                i += '</div>';
            }
            i += '</div>';
